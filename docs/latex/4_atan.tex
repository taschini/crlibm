...
(\ref{eq:poly_eva1l}) and (\ref{eq:arctan_redu}) in double-double
arithmetic.
...
\end{lstlisting}
\begin{tabular}{ll}
Line 1 & The error between $\mathrm{Xred2}$ and the ideal value of $\xred^2$\\
...
The reconstruction adds $\arctan(b_i)$ that is read as a double-double
from a table, and $\arctan(\xred)$, approximated by
