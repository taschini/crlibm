...
\subsubsection{Methods to raise IEEE-754 flags}
The IEEE standard impose, in certain cases, to raise flags and an exception for the $4$ operators ($+$, $\times$, $\div$, $\sqrt{~}$). Therefore, it is legitimate to require the same for elementary functions.

The code of our program intends to be portable, that is done by using ANSI-C99 language. Then we can't generate these exceptions manually with assembly code; we have to let the set composed of the processor, compiler and system to generate exceptions and flags using the following instructions :

\begin{itemize}
\item {\bf underflow} : the multiplication $\pm smallest \times smalest$ where $smallest$ correspond to the smallest denormalized number,
...
\endlastfoot 
\ref{exp:code:1} & Put the high part of $x$ in $hx$. (cf. prog. \ref{chap0:lst:endian})
\\
...
if (k != 0){ 
  ...
  rp_hi = x-ln2_hi*k;
  rp_lo =  -ln2_me*k;
  Fast2SumCond(r_hi, u, rp_hi, rp_lo);         (*@ \label{exp:code:12} @*)
...
\end{prop}
\\
\ref{exp:code:11} & Put in $k$ the closest integer of $x*inv\_ln2$. We use the property of DOUBLE2INT that convert a floating-point number in rounding to nearest mode (program \ref{chap0:lst:conversion2}, page \pageref{chap0:lst:conversion2}).
...
By the Sterbenz theorem (theorem \ref{chap0:theo:sterbenz}, page \pageref{chap0:theo:sterbenz}), we have 
...
We use conditional Fast2Sum algorithm (with tests on entries), because $x-ln2\_hi \times k$ can be equal to zero (due to the $58$ bits of cancellation). The conditional Fast2Sum algorithm (program \ref{chap0:lst:fast2sumcond}, page \pageref{chap0:lst:fast2sumcond}) leads to 
...
Fast2Sum(rp_hi, rp_lo, r_hi, r_lo)   (*@ \label{exp:code:25} @*)
...
This sequence of instructions is similar to the one used within DOUBLE2INT (program \ref{chap0:lst:conversion2}, page \pageref{chap0:lst:conversion2}). It puts in $index$ variable, bits of weight $2^0$ to $2^{-8}$, minus the value of the bias. Meanwhile, it puts in $index\_flt$ the floating-point value corresponding to the first $8$ bits of $r\_hi$.
...
\\
\ref{exp:code:25}&
The Fast2Sum algorithm guarantee :
\begin{prop}
...
Dekker(R3_hi, R3_lo, ex_hi, rp_hi);        (*@ \label{exp:code:32} @*)
R4 = ex_hi * rp_lo;                        (*@ \label{exp:code:33} @*)

Dekker(R5_hi, R5_lo, ex_hi, R1);           (*@ \label{exp:code:34} @*)
R6 = R4 + (ex_lo * (R1 + rp_hi));          (*@ \label{exp:code:35} @*)

R7  = ex_hi * R2;                          (*@ \label{exp:code:36} @*)
R7 += (R6 + R5_lo) + (R3_lo + ex_lo);      (*@ \label{exp:code:37} @*)

Fast2Sum(R9, R8, R7, R5_hi);               (*@ \label{exp:code:38} @*)

Fast2Sum(R10, tmp, R3_hi, R9);             (*@ \label{exp:code:39} @*)
R8 += tmp;                                 (*@ \label{exp:code:40} @*)

Fast2Sum(R11, tmp, ex_hi, R10);            (*@ \label{exp:code:41} @*)
R8 += tmp;                                 (*@ \label{exp:code:42} @*)

Fast2Sum(R11, R8, R11, R8);                (*@ \label{exp:code:43} @*)
...
\end{prop}\\
\ref{exp:code:32}&
By using Dekker algorithm (programme \ref{chap0:lst:Dekker0}, page
\pageref{chap0:lst:Dekker0}) and properties  \pref{chap3:exp:rphi_rplo} and \pref{chap3:exp:ex_hilo} we have :
\begin{prop}
...
\end{prop}\\
\ref{exp:code:34}&
By using Dekker algorithm and properties \pref{chap3:exp:ex_hilo} and \pref{chap3:exp:prop30} we have :
\begin{prop}
...
% 
    \input{exp_fig/reconstruction.pstex_t}
...
if the result do not represent a denormalised number. Indeed, if the final results belong to denormalised numbers, then the precision of the results is less than the $53$ bits of a normalized nomber. Lets take an exemple. Let $a=1.75$ be a floating-point number exactly representable in double precision format (we have $\circ(a) = a$). Let multiply this number by $2^{-1074}$, the exact result is $1.75 \times 2^{-1074}$ which is different of the result rounded to nearest in double precision $\circ(1.75 \times 2^{-1074})=2^{-1073}$. 
...
if (R11 == (R11 + R8 * errn)){             (*@ \label{exp:code:44} @*)
  if (k > -1020){                          (*@ \label{exp:code:45} @*)
    ...
    HI(st2mem) = R12.i[HI_ENDIAN];         (*@ \label{exp:code:49} @*)
    LO(st2mem) = R12.i[LO_ENDIAN];         (*@ \label{exp:code:50} @*)
...
\begin{figure}[ht] \begin{center}
\input{exp_fig/rnd_to_nearest.pstex_t}
...
void exp_SC(scs_ptr res_scs, double x){
  ...
  db.i[HI_ENDIAN] -= (9 << 20);                (*@ \label{exp:mp:1} @*) 
...
We have :
\begin{itemize}
\item $P_0=c_1 \boxplus (red \boxtimes c_0)$   donc $|P_0| \leq 2^{-20}$ et $P_0 = c_1 + (red \times c_0 ) + \epsilon_{-231}$
\item $P_1=c_2 \boxplus (red \boxtimes P_0)$   donc $|P_1| \leq 2^{-17}$ et $P_1 = c_2 + (red \times P_0 ) + \epsilon_{-228}$
\item $P_2=c_3 \boxplus (red \boxtimes P_1)$   donc $|P_2| \leq 2^{-14}$ et $P_2 = c_3 + (red \times P_1 ) + \epsilon_{-225}$
\item $P_3=c_4 \boxplus (red \boxtimes P_2)$   donc $|P_3| \leq 2^{-11}$ et $P_3 = c_4 + (red \times P_2 ) + \epsilon_{-222}$
\item $P_4=c_5 \boxplus (red \boxtimes P_3)$   donc $|P_4| \leq 2^{-8}$ et $P_4 = c_5 + (red \times P_3 ) + \epsilon_{-219}$
\item $P_5=c_6 \boxplus (red \boxtimes P_4)$   donc $|P_5| \leq 2^{-5}$ et $P_5 = c_6 + (red \times P_4 ) + \epsilon_{-216}$
\item $P_6=c_7 \boxplus (red \boxtimes P_5)$   donc $|P_6| \leq 2^{-3}$ et $P_6 = c_7 + (red \times P_5 ) + \epsilon_{-214}$
\item $P_7=c_8 \boxplus (red \boxtimes P_6)$   donc $|P_7| \leq 2^{-1}$ et $P_7 = c_8 + (red \times P_6 ) + \epsilon_{-212}$
\item $P_8=c_9 \boxplus (red \boxtimes P_7)$   donc $|P_8| \leq 1 + 2^{-10} $ et $P_8 = c_9 + (red \times P_7 ) + \epsilon_{-211}$
\item $P_9=1   \boxplus (red \boxtimes P_8)$   donc $|P_9| \leq 1 + 2^{-9}$ et $P_9 = 1   + (red \times P_8 ) + \epsilon_{-210}$
\item $P_{10}=1   \boxplus (red \boxtimes P_9)$donc $|P_{10}| \leq 1 + 2^{-8}$ et $P_{10} = 1   + (red \times P_9) + \epsilon_{-210}$
\end{itemize}
...
Tables \ref{tbl:timings_norm} gives a summary of the timings of the
various libraries. The timings are normalized to the average time of
the default \texttt{libm} on each system, which should be taken with
care as the \texttt{libm}s used by default on the tested systems are
all different. Table \ref{tbl:abstime} gives absolute times.
...
\hline
 &\multicolumn{2}{|c||}{default libm} &   \multicolumn{3}{|c|}{correctly rounded libraries}          \\
\cline{2-6}
Processor           &\# errors& time &{crlibm}&{MPFR}&{libultim}       \\ \hline\hline

             &       & 1     & 1.21   & 45   & 0.9      \\ \cline{3-6}
\raisebox{1.5ex}[0pt]{Pentium III}&\raisebox{1.5ex}[0pt]{${1}/{587}$}
                     & 1.81     & 39.8     & 93.8     & 3339     \\ \hline\hline

             &       & 1     & 2.6    & 153  & 1.8      \\ \cline{3-6}
\raisebox{1.5ex}[0pt]{Itanium}&\raisebox{1.5ex}[0pt]{${1}/{491}$}
                     & 13.7     & 46     & 165.3  & 14499    \\ \hline\hline

             &       & 1     & 2.66   & 83   & 1.2      \\ \cline{3-6}
\raisebox{1.5ex}[0pt]{UltraSparc IIi}&\raisebox{1.5ex}[0pt]{${1}/{41}$}
                     & 12.9     & 147    & 383  & 3576     \\ \hline\hline

             &       & 1     & 0.91   & 13.5 & 0.93     \\ \cline{3-6}
\raisebox{1.5ex}[0pt]{PowerPC G4}&\raisebox{1.5ex}[0pt]{${1}/{4739}$}
                     & 1.32     & 9.25     & 26.9 & 1477     \\ \hline

\end{tabular}
\end{center}
\caption{
  Accuracy and timings for the exponential function from various libraries. 
  Timings are normalized to the average time for the default \texttt{libm}. 
  For each processor, the first line gives the average time, 
  and the second line gives the worst-case time.
  \label{tbl:timings_norm}}
\end{table}
...
\hline
Processor &  & libm   & crlibm & MPFR  & libultim\\ \hline \hline
             & average time    & 462    & 562    & 21114 & 413     \\ \cline{2-6}
Pentium III (cycles) & overall worst-case time  & 837    & 18413  & 43316 & 66050   \\ \cline{2-6}
             & correct rounding worst case time & 448    & 15963  & 38969 & 1542415 \\ \hline\hline

             & average time     & 982    & 1178   & 24991 & 942     \\ \cline{2-6}
Xeon (cycles)        & overall worst-case time  & 3124   & 34236  & 138768& 108920  \\ \cline{2-6}
             & correct rounding worst case time & 1080   & 30384  & 50436 & 2592660 \\ \hline\hline

             & average time     & 202    & 518    & 30995 & 371     \\ \cline{2-6}
Itanium (cycles)     & overall worst-case time  & 2767   & 9349   & 70660 & 139415  \\ \cline{2-6}
             & correct rounding worst case time & 131    & 6434   & 33388 & 2928718 \\ \hline\hline

             & average time     & 762    & 2033   & 63570 & 950     \\ \cline{2-6}
UltraSparc IIi (cycles) &overall worst-case time  & 9823   & 112366 & 292129& 157987  \\ \cline{2-6}
             & correct rounding worst case time & 292    & 91190  & 126827& 2724912 \\ \hline\hline

             & average time     & 2.27   & 2.07   & 30.7  & 2.1     \\ \cline{2-6}
PowerPC G4 (ms)  & overall worst-case time  & 3      & 21     & 61    & 116     \\ \cline{2-6}
             & correct rounding worst case time & 2      & 20     & 59    & 3354    \\ \hline
\end{tabular}
\end{center}
\caption{Absolute timings for the exponential
  \label{tbl:abstime}}
\end{table}
...
For some of our test, we have disconnected the call to the second
phase in the evaluation scheme and counted  one miss-rounding result
over $2097152$ ($\approx 2^{21}$). As we can see in the first column
of table \ref{tbl:timings_norm}, it mean that we are far more precise
than others library.
...
average  means that a possible performance improvment
