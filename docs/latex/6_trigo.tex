...
the input number into a 32-bit integer. This will enable faster
comparisons. However we now compare to integer constants and we have
to be aware that we lost the lower part of \texttt{x}, which had a
value up to $(2^{31}-1)\ulp(x)$. This is taken care of in the
procedure that converts a Maple high-precision number into such an
integer.
