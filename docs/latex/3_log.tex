...
For this function, the search for worst-case accuracy required to
compute in double precision the correctly rounded  logarithm of a double is
completed. This worst-case accuracy is $2^{-118}$ according to Lefèvre
and Muller \cite{LefMul2004}.

We therefore proceed in two phases \cite{Ziv91}.  The first, quick
phase (program \texttt{log\_fast.c}) is accurate only to $2^{-57}$ and
$2^{-64}$, depending on the execution path as detailed below.  If this
is not enough to decide correct rounding, a second phase, accurate to
$2^{-120}$,  using the SCS library is launched (program
\texttt{log\_accurate.c}).
...
Concerning subnormals, the logarithm function cannot return a subnormal number: it comes close to $0$  as $x$ comes close to $1$ (as $\log(1+x) \approx x$ when $x\rightarrow 0$), and the double-precision floating-point numbers closest to $1$ are $1+2^{-52}$ and $1-2^{-53}$. Therefore the smallest possible exponent of the logarithm of a double is $-53$.

%the smallest exponent of the logarithm of a double-precision input number is $-53$ (for the input values $\log(1+2^{-52})$ and $\log(1-2^{-52})$, as $\log(1+\epsilon) \approx \epsilon$ when $\epsilon\rightarrow 0$). This will make it easy to ensure that no subnormal ever appears in the computation of the logarithm of a double-precision input number.
...
\begin{enumerate}
\item Handling of subnormal number:\\
If $x < 2^{-1022}$ then
$$\log(x) = -52 * \log(2) + \log\left(\frac{x}{2^{-52}}\right)$$

\item Argument reduction:\\
Computes the reduced argument $y$ such that $x=2^E.y$ with $E$ and 
\begin{equation}
\frac{11}{16}<\frac{\sqrt{2}}{2} < y < \sqrt{2}<\frac{23}{16} \quad.
\end{equation}

\item Polynomial evaluation:\\
The interval $[\frac{11}{16},\frac{23}{16}]$ is broken down into
8 intervals given in Table~\ref{table:TablePolysLog1}. On each interval the choosen polynomial are evaluated thanks to a Horner scheme:
...
\item Reconstruction:\\
Compute in double-double precision arithmetic :
...
First we need to handle subnormal inputs: If $x < 2^{-1022}$ , ie if x
...
\hline  
P[0] & $[\frac{11}{16},\frac{12}{16}]$ &  $\frac{23}{32}$ & $<2^{-5}$  &  65.17  \\ 
\hline                                                                                
...
\hline                                                                                
P[7] & $[\frac{21}{16},\frac{23}{16}]$ &  $\frac{22}{16}$ & $<2^{-4}$  &  62.25  \\ 
\hline
...
subnormal ever appear in the Horner evaluation of a polynomial. This
...
The rounding tests need a bound on the relative error for each interval, which can be computed as follows:
...
\begin{itemize}
\item The absolute value of $P(z)$ is bounded by $0.346=\log(\sqrt{2})$. A bound on
...
\item If $E=0$ then the reconstruction step is skipped, and the error
  ...
  The double-double part of the polynomial evaluation is usually
  started with an addition as we noticed in section \ref{sec:Horner}, page
  \pageref{sec:Horner}. However, it turns out that the polynomial $P[5]$ (the
  one which is centered on $1$ and has therefore a null coefficient of
  degree 0) leads to the worst relative rounding error. To keep an
  acceptable percentage of accurate phase in this case, we chose to
  exceptionnaly start the double-double part of the polynomial
  evaluation with a multiplication rather than the usual addition.
...
\begin{table}[!h]
  \begin{center}
    \begin{tabular}{|c|c||c|c|c|c|}
      ...
      \hline
P[1] & .347 & 62.76 & 61.78& 67.34  & 64.17\\ 
P[2] & .288 & 60.81 & 60.83& 66.16  & 63.29\\ 
P[3] & .208 & 60.02 & 61.11& 66.19  & 63.14\\ 
P[4] & .134 & 58.96 & 61.39& 66.27  & 63.34\\ 
P[5] & .065 & 57.63 & 60.04& 64.76  & 62.39\\ 
P[6] & .172 & 57.94 & 60.15& 65.13  & 63.01\\ 
P[7] & .272 & 59.89 & 60.23& 65.51  & 63.05\\ 
P[8] & .347 & 60.80 & 60.46& 66.02  & 63.28\\ 
      \hline
      \hline
    \end{tabular}
  \end{center}
...
Here is the listing of  \texttt{log\_rn}. 
...
\label{pageref:log_of_one}
For the other functions it is almost identical, with one subtlety: for
the input value exactly exact to $1$, the procedure
\texttt{log\_quick} does return exactly zero as a double-double.
However, in this case, the rounding test of macro
\verb!TEST_AND_RETURN_RD! launches the SCS computation, which
(correctly) returns a number smaller than $2^{-150}$, which is
(correctly) rounded down to the smallest representable negative
number. Of course, the value to be returned should be zero. This
problem happens because $\log(1)$ is exactly representable as a double
(fortunately, $1$ is the only double with this property). To avoid
this, we add a test for $x=1$ to the filtering of exceptional cases in
...
Line 29 & This test is equivalent to $|ln2\_times\_E_HI| < \mathtt{MIN\_FASTPATH}$.\\
        & As $|E|$ is an integer smaller than $2048$, neither overflow nor subnormal can occur.\\
...
Lines 34,36,47 & None of the intermediate values reach  $2^{970}$ so we may use the unconditional Mul22.\\
...
Therefore the function  \texttt{scs\_log} takes as arguments $y$ and $E$, computed
...
The polynomials are available in \texttt{log\_accurate.h}.
...
\hline
 ...
 \hline
 \texttt{libm}           & 180          & 6612          &        193 \\ 
 \hline
  \texttt{mpfr}          & 8            & 266808        &      46698 \\ 
 \hline
  \texttt{libultim}      & 440          & 498976        &        521 \\ 
 \hline
 \texttt{crlibm}         & 332          & 52392         &        528 \\ 
 \hline
 \hline
\multicolumn{4}{|c|}{Pentium III / Linux Debian woody / gcc 3.0}   \\
 \hline
  \hline
                         & min time      & max time      & avg time \\ 
 \hline
 \texttt{libm}           & 184          & 699           &        184 \\ 
 \hline
  \texttt{mpfr}          & 239          & 184801        &      51279 \\ 
 \hline
  \texttt{libultim}      & 182          & 201400        &        293 \\ 
 \hline
 \texttt{crlibm}         & 173          & 19404         &        319 \\ 
 \hline
