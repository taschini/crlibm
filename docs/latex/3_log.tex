...
around $1$, so if needed the mantissa is possibly divided by 2 (and
...
and $c1 = c_1^{hi} + c_1^{lo}$. Their relative approximation error is
...
bits of its mantissa equal to zero. Therefore, the computation of
...
\item If $E\ne0$, then the result is computed as $E\times \log(2) +
  ...
  value attainable value of the result:
...
operating systems.  The input numbers for these tests are random
positive double-precision numbers with a normal distribution on the
exponents. More precisely, we take random 63-bit integers and casted
...
\hline
 \texttt{crlibm} (without FMA)        & 5           & 1144          &         26 \\
