...
\begin{itemize}
\item {\bf underflow} : the multiplication $\pm smallest \times smallest$ where $smallest$ correspond to the smallest subnormalized number,
...
\item {\bf inexact} : the addition $(x + smallest) - smallest$ where $x$ is the result and  $smallest$ the smallest subnormalized number,
...
trigonometric argument reduction (chapter \ref{chap3:log}, \pageref{chap3:log}).
...
error $\epsilon$. This correspond to detect whether we are in a hard
to round case.
...
Let $y$ be a real number, and  $\maxeps$, $e$, $y_h$ and $y_l$ be
  ...
  \begin{itemize}
  \item Non overlapping: $y_h=y_h\oplus y_l$,
  \item equality required by ieee-754 for NaN: none of $y_h$ and $y_l$ is a  NaN.
  \item non-underflowing numbers: $|y_h|\ge 2^{-1022+53}$ 
  \item definition of the error term: $|y_h+y_l - y| <\maxeps.|y|$
  \item error greater than the precision of one dp number: $0<\maxeps \le 2^{-53-k}$ with $k\ge 2$ integer
  \item condition over the constant $e$: $e\ge 1+  \dfrac{2^{53+k+1}\maxeps}{(2^{k}-1)(1-2^{-53})}$
\end{itemize}
...
\begin{proof}
  ...
  It also holds if $y_h$ is a subnormal: In this case $y_l$ is zero, so
...
\begin{proof}
  ...
  that we cannot ensure anymore that $y_l$ will not be a subnormal.
...
It is important to note that all this doesn't work if $y_h$ is a subnormal. If
