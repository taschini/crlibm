  ...
  $$ \left\{
    \begin{array}{rl}
      p_{k}  &= x \otimes s_{k+1}
             \ = \ \hat{x}.S_{k+1}(\hat{x}) (1+\epsilon'_k)\\
      s_{k}  &=  c_k \oplus p_k
             \ = \ S_{k}(\hat{x}) + \delta_k\\
      \end{array}
  \right.
...
The following computes tight bounds on $\epsilon'_k$ and on $\delta_k$.
...
\item Horner steps: 
  ...
  \item multiplication step:%  $p_{k}  \ = \ x \otimes s_{k+1} \ = \ P_{k}(\hat{x})(1+\epsilon'_k)$
    \begin{eqnarray*}
      p_{k}  &=& x \otimes s_{k+1} \\
             &=& \hat{x}(1+\epsilon_x)\ \otimes \ (S_{k+1}(\hat{x}) + \delta_{k+1})\\
             &=& \hat{x}.S_{k+1}(\hat{x})(1+\epsilon_x)(1 + \frac{\delta_{k+1}}{S_{k+1}(\hat{x})})(1+\epsilon_\otimes)\\
    \end{eqnarray*}
    We therefore get 
    \begin{equation}
...
\item If $c_0\ne 0$, for small values of $x$, a good bound on the
  ...
  sub-intervals, and we compute $\delta_k$, $\mins{0}$ and $\rho$ on
