...
Cody and Waite algorithm, used in the $\log$ and in the trigonometric
argument reduction.
...
recursion:
  ...
  For fast and accurate function evaluation, we try to have
  $|x|<\maxx$ where $\maxx$ is small with respect to the coefficients.
  In this case the error in one step is scaled down for the next step
  by the multiplication by $x$, allowing for an accumulated overall
  error which is actually close to that of the last operation.
...
\item $\delta_\oplus$ and $\delta_\otimes$ the absolute error when
  ...
  begins in double-precision ($\maxeps_\oplus=\maxeps_\otimes=53$) and ends in double-double
  ($\maxeps_\oplus=\maxeps_\otimes=102$).
...
\item $p_j = x \otimes s_j $  the result of a multiplication step in
  the Horner scheme. We recursively evaluate its error $\maxeps'_j$
  with respect to the exact mathematical value $P_j(\hat{x})=xS_{j+1}(\hat{x})$.

\item $s_j = c_j \oplus p_{j+1}$ (with $s_d = c_d$)  the result of
...
\item Horner steps: 
  \begin{itemize}
  \item multiplication step:%  $p_{k}  \ = \ x \otimes s_{k+1} \ = \ P_{k}(\hat{x})(1+\epsilon'_k)$
    \begin{eqnarray*}
      p_{k}  &=& x \otimes s_{k+1} \\
             &=& \hat{x}(1+\epsilon_x)\ \otimes \ S_{k+1}(\hat{x})(1 + \epsilon_{k+1})\\
             &=& \hat{x}.S_{k+1}(\hat{x})(1+\epsilon_x)(1 + \epsilon_{k+1})(1+\epsilon_\otimes)\\
    \end{eqnarray*}
    We therefore get 
    \begin{equation}
    ...
    with 
    \begin{equation}
      \maxeps'_k \ = \ (1+\maxeps_x)(1 + \maxeps_{k+1})(1+\maxeps_\otimes) - 1 \label{eq:maxepsp}
    \end{equation}
  ...
  \item addition step 
      \begin{eqnarray*}
        s_{k} &=&  c_k \oplus p_k \\
              ...
              &=&  c_k + P_k(\hat{x})(1+\epsilon'_k) + \delta_\oplus\\
              &=&  c_k + P_k(\hat{x}) + \epsilon'_k P_k(\hat{x})  + \delta_\oplus\\
              &=&  S_{k}(\hat{x})\  +\  \epsilon'_k P_k(\hat{x}) +  \delta_\oplus \\
      \end{eqnarray*}
    ...
    We therefore get 
      ...
      \begin{equation}
        \maxdelta_k  \ = \  \maxeps'_k\infnorm{P_{k}} + \maxdelta_\oplus\label{eq:maxdelta}
      \end{equation}
      
      Here $\maxdelta_\oplus$ will be computed for double-precision operations as 
      $$\maxdelta_\oplus = \frac{1}{2}\ulp(S_k(\hat{x}) + \epsilon'_k
      P_k(\hat{x}))\quad .$$
      For double-double or SCS operations, $\maxdelta_\oplus$ will be computed as
      $$\maxdelta_\oplus=2^{-\nu}(\infnorm{S_{k}} + \maxeps'_k\infnorm{P_{k}})$$
      with $\nu=102$ and $\nu=205$ respectively.
    \end{itemize}
  \end{itemize}
...
\item If $c_0\ne 0$, for small values of $x$, a good bound on the
  ...
  sub-intervals, and we compute $\delta_k$, $\mins_0$ and $\rho$ on
...
~\\
Let $y$ be a real number, and  $\epsilon$, $e$, $y_h$ and $y_l$ be
  ...
  \item $e\ge 1+  \dfrac{2^{53+k+1}\epsilon}{(2^{k}-1)(1-2^{-53})}$
\end{itemize}
...
\begin{proof}
  ...
  \round(y)$. We will prove that  if the test is true, then $|y_l-\frac{1}{2}u| > 2^{53}\epsilon
  ...
  If $0 \le y_l < (\frac{1}{2} - \frac{1}{2^{k+1}})u$, then since
    $|\epsilon|<2^{-53+k}$ we are in an easy case for rounding to the
...
(\ref{eq:prooftestRN2}) implies
    ...
    2^{53}\epsilon u$, from $e\ge 1+
    \frac{2^{53+k+1}\epsilon}{(2^{k}-1)(1-2^{-53})}$.
...
~\\
  Let $y$ and $\epsilon$ be real numbers, and $e$, $y_h$ and $y_l$ be
  floating-point numbers such that 
  \begin{itemize}
  \item $y_h=y_h\oplus y_l$,
  \item none of $y_h$ and $y_l$ is a  NaN.
  \item $|y_h+y_l - y| <maxeps.|y|$
  \item $0<maxeps \le 2^{-53-k}$ with $k\ge 2$ integer
  \item $e\ge 1+  \dfrac{2^{53+k+2}\epsilon}{2^{k}-1}$
\end{itemize}
...
\begin{itemize}
\item In general we will target values of $\epsilon$ in the order of
...
~\\
Let $y$ be a real number, and $y_h$, $y_l$ and $\epsilon$ be
floating-point numbers such that
  \begin{itemize}
  \item $y_h=y_h\oplus y_l$,
...
\begin{proof}
  ...
  two, is exact), hence $y_l>\epsilon y$ so we are in an easy case for directed rounding.
...
\subsection{Accumulated rounding error in Horner evaluation \label{sec:Horner-maple}}
