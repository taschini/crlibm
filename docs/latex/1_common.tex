...
Let $y$ be a real number, and  $\maxeps$, $e$, $y_h$ and $y_l$ be
  ...
  \item $|y_h+y_l - y| <\maxeps.|y|$ (i.e. the total relative error of
    $y_h+yl$ with respect to $y$ is bounded by $\maxeps$),
  \item $0<\maxeps \le 2^{-53-k}$ with $ k \ge 3$ integer,
  \item TODO: wrong $e\ge 1+  \dfrac{2^{53+k+1}\maxeps}{(2^{k}-1)(1-2^{-53})}$.
\end{itemize}
...
\begin{proof}
  ...
  is a normal number. We will assume that $y\ge0$ (so $y_h\ge0$), as
  the other case is symmetric.
  
  We want to prove is that if the test is true, then $y_h = \round(y)$.


  First some lemmata.

  Let us note $u=\ulp(y_h)$. By definition of the \ulp\ of a normal
  ...
  \begin{equation}
    ...
    \label{eq:prooftest1}
  \end{equation}
  Therefore  $y_h+y_l \le (2^{53}-1)u + \frac{1}{2}u\ <\ 2^{53}u$:
  \begin{equation}
    y_h+y_l < 2^{53}u
    \label{eq:prooftest2}
  \end{equation}

  Since $1+\epsilon>\frac{1}{2}$, we also have
  $y=\frac{y_h+y_l}{1+\epsilon} < 2^{54}u$. 

  From $\maxeps<2^{-53-k}$ (fifth hypothesis) and $y < 2^{54}u$ we derive  
  \begin{equation}
    \maxeps y < 2^{-k+1}u
    \label{eq:prooftest3}
  \end{equation}
  and $k>2$ gives $\maxeps y < 2^{-1}u$ which in turn implies   
  $y < y_h + y_l +\maxeps y < (2^{53}-1)u +\frac{1}{2}u +\frac{1}{2}u$ :
  \begin{equation}
    y<2^{53}u .
    \label{eq:prooftest4}
  \end{equation}

  First we consider the (general) case when $y_h$ is not a power of two.  In
  this case we will prove that if the test is true, then
  $||y_l|-\frac{1}{2}u| > 2^{53}\maxeps u > \maxeps y$, which implies
  that we are not in a difficult case for correct rounding to the
  nearest, so $y_h = \round(y)$.

  We consider only the case when $y_l$ is positive (the other case is
  symmetrical): $0\le y_l \le \frac{1}{2}u$.
  ...
  distant from $u/2$. More specifically, if $0 \le y_l < u/2 -
  2^{-k+1}u$, we rewrite $y_h+y_l-y=\epsilon y$ as $y-y_h = y_l
  - \epsilon y < u/2 - 2^{-k+1}u - \epsilon y $. Since
  $\epsilon < \maxeps$ and $\maxeps y < 2^{-k+1}u$ from
  (\ref{eq:prooftest3}), we get $y-y_h < u/2$, therefore $y_h
  = \round(y)$.

  Now consider the case when $y_l \ge (\frac{1}{2} -
  \frac{1}{2^{k-1}})u$.  The condition $|y_h|\ge
  2^{-1022+52}$ ensures that $\frac{1}{4}u$ is a normal number, and now $y_l > u/4$, so  in
  this case $y_l$ is a normal number. As $e>1$ this ensures that
  ...
  \otimes e| \le \frac{1}{2}u$, which in turn implies $|y_l \times e|
  (1-2^{-53}) \le \frac{1}{2}u$, as $y_l$ is a normal number and
  $e>1$.
  
  We get $y_l e (1-2^{-53}) \le \frac{1}{2}u$, or (using $y_l \ge (\frac{1}{2} -
  \frac{1}{2^{k-1}})u$)
  \begin{equation}
  \frac{1}{2}u - y_l \ge (\frac{1}{2} -
  \frac{1}{2^{k-1}})u (e(1-2^{-53})-1) \label{eq:prooftestRN2}
  \end{equation}  

  The condition that $e$ has to fullfil (last hypothesis) was designed
  so that (\ref{eq:prooftestRN2}) implies $\frac{1}{2}u - y_l >
  2^{53}\maxeps u > \maxeps y$: therefore $y_h$ is the correctly
  rounded to the nearest of $y$.

  The case when $y_l$ is negative is similar.

  Now consider the (very rare) case when $y_h$ is a power of two. $y_h=2^{52}u$.
