...
\item Demand C99 compliance (pass the \texttt{-C99} flag to the compiler). For Fortran, demand F90 compliance.
\item Call \texttt{crlibm\_init()} before you begin floating-point
  ...
  IEEE-754 double-precision with round to nearest mode. On IA32
  processors, problems may still occur for extremely large or extremely small
  values.
\item Do not hesitate to rely heavily on parentheses (the compiler
  should respect them according to the standards, although some
  don't). Many times, wondering where the parentheses should go in an
