...
#rounding constant
rnd_cst := max(epsilon_cosh_35_inf, epsilon_cosh_0_35, delta_cosh):
k_cst_cst := floor(-log2(rnd_cst))-53:
round_cst_cosh := 1+2^(53+k_cst_cst+1)*rnd_cst / ((2^k_cst_cst-1)(1-2^(-53)));
...
for big_little from 1 to 2 do
	...
	fprintf(fd, cat(  "  db_number const two_minus_30 = ", IEEE2db_number(2**(-40),big_little))):
